...
int main(int argc, char* argv[])
{
  ...
  while ((c = getopt (argc, argv, "f:n:h")) != -1)
    switch (c)
      ...
      case 'n':
        ...
        break;
      case 'h':
        usage("makeMiniNtuple_Stop2L");
        return 1;
      case '?':
        if (optopt == 'f')
          fprintf (stderr, "makeMiniNtuple_Stop2L\t Option -%c requires an argument.\n", optopt);
        else if (optopt == 'n')
          fprintf (stderr, "makeMiniNtuple_Stop2L\t Option -%c requires an argument.\n", optopt);
        else if (isprint (optopt))
          fprintf (stderr, "makeMiniNtuple_Stop2L\t Unknown option `-%c'.\n", optopt);
        else
          fprintf (stderr,
                   "makeMiniNtuple_Stop2L\t Unknown option character `\\x%x'.\n",
                   optopt);
        return 1;
      default:
        abort ();
  ...
  for (int index = optind; index < argc; index++)
    printf ("makeMiniNtuple_Stop2L\t Non-option argument %s\n", argv[index]);
  if (input_file==NULL) {
    printf("makeMiniNtuple_Stop2L\t An input file must be provided with option -f (a list, a DIR or single file)\n");
    return 0;  
  }
  

  // Print information
  printf("makeMiniNtuple_Stop2L\t =================================================================\n");
  printf("makeMiniNtuple_Stop2L\t Running SusyAnalysis/makeMiniNtuple_Stop2L\n");
  printf("makeMiniNtuple_Stop2L\t =================================================================\n");
  printf("makeMiniNtuple_Stop2L\t   Flags:\n");
  printf("makeMiniNtuple_Stop2L\t     Input file (-f)       : %s\n",input_file);
  printf("makeMiniNtuple_Stop2L\t     Number of events (-n) : %i\n",n_events );
  printf("makeMiniNtuple_Stop2L\t =================================================================\n");
  ///////////////////////////////////////////////////////////////////////
  ...
  if(inputIsFile) {
    ...
    printf("makeMiniNtuple_Stop2L\t Cannot understand input %s",input_file);
    return 0;  
  }
  ...
  printf("makeMiniNtuple_Stop2L\t Total events available : %lli\n",chain->GetEntries());
  ...
  *cutflow << [&](Superlink* /*sl*/, var_void*) { 
  ...
  // Print information
  printf("makeMiniNtuple_Stop2L\t =================================================================\n");
  printf("makeMiniNtuple_Stop2L\t All done!\n");
  printf("makeMiniNtuple_Stop2L\t =================================================================\n");
  return 0; 
}
